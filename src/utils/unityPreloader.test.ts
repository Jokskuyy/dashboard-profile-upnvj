...
function getUnityFileUrls(basePath = '/'): string[] {
  const buildPath = `${basePath}unity-builds/v0.6/Build`;
  return [
    `${buildPath}/v0.6.loader.js`,
    `${buildPath}/v0.6.framework.js.unityweb`,
    `${buildPath}/v0.6.wasm.unityweb`,
    `${buildPath}/v0.6.data.unityweb`,
  ];
}
...
describe('UnityPreloader — download priority order (behavior)', () => {
  ...
  it('all URLs reference v0.6 build', () => {
    const urls = getUnityFileUrls();
    urls.forEach(url => {
      expect(url).toContain('v0.6');
    });
  });
});
