...
function getUnityFileUrls(basePath = '/'): string[] {
  const buildPath = `${basePath}unity-builds/V0.6.7/Build`;
  return [
    `${buildPath}/V0.6.7.loader.js`,
    `${buildPath}/V0.6.7.framework.js.unityweb`,
    `${buildPath}/V0.6.7.wasm.unityweb`,
    `${buildPath}/V0.6.7.data.unityweb`,
  ];
}
...
describe('UnityPreloader — download priority order (behavior)', () => {
  ...
  it('all URLs reference V0.6.7 build', () => {
    const urls = getUnityFileUrls();
    urls.forEach(url => {
      expect(url).toContain('V0.6.7');
    });
  });
});
