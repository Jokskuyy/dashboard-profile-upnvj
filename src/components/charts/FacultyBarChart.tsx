    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="bg-gray-50 rounded-lg p-2 sm:p-4">
        <div className="flex w-full">
          {/* Y-Axis */}
          <div
            className="flex flex-col justify-between mr-2 sm:mr-4"
            style={{ height: chartHeight }}
          >
            {yAxisValues.reverse().map((value, index) => (
              <div
                key={index}
                className="text-[10px] sm:text-xs text-gray-500 text-right w-6 sm:w-8"
              >
                {value}
              </div>
            ))}
          ...
          <div className="flex-1">
            ...
            <div className="relative" style={{ height: chartHeight }}>
              ...
              {/* Bars Container */}
              <div className="absolute inset-0 flex items-end px-1 sm:px-4">
                {data.map((faculty, index) => {
                  const barHeight =
                    (faculty.count / maxCount) * (chartHeight - 20);
                  const isSelected = selectedFaculty === faculty.id;
                  const totalBars = data.length;
                  // Responsive bar width - otomatis sesuai jumlah bars, tidak overflow
                  const containerWidth = typeof window !== "undefined" ? window.innerWidth : 768;
                  const availableWidth = containerWidth < 640 ? containerWidth - 100 : containerWidth * 0.8;
                  const barWidth = Math.max(20, Math.min(50, (availableWidth / totalBars) * 0.6));
            ...
            {/* X-Axis Labels */}
            <div
              className="flex mt-4 px-1 sm:px-4 relative"
