...
import { Users, BarChart3, ArrowLeft, BookOpen, Mail, User, ChevronRight } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';
import { professorsData, getProfessorsByFaculty, getDepartmentsByFacultyId, getProfessorsByDepartment } from '../utils/staticData';
...
const ProfessorsSection: React.FC = () => {
  ...
  const departmentProfessors = selectedDepartmentData 
    ? getProfessorsByDepartment(selectedDepartmentData.name)
    : [];
    
  const displayedProfessors = selectedDepartmentData 
    ? (showAll ? departmentProfessors : departmentProfessors.slice(0, 3))
    : [];
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {selectedDepartmentData 
              ? `${selectedDepartmentData.professors} ${t('professorsInDepartment')}`
              : selectedFacultyData 
      ...
      {!selectedFaculty ? (
        ...
        <div>
          ...
          <div className="flex items-center mb-4">
            ...
            <span className="text-sm font-medium text-gray-700">{t('departmentDistribution')}</span>
          </div>
          <DepartmentBarChart
        ...
        <div>
          ...
          <div className="bg-gray-50 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <Users className="w-4 h-4 mr-2" style={{ color: selectedDepartmentData?.color }} />
                    <span className="text-sm font-medium text-gray-700">
                      {t('professors')}
                    </span>
                  </div>
                  <span className="text-lg font-bold" style={{ color: selectedDepartmentData?.color }}>
                    {selectedDepartmentData?.professors}
                  </span>
                </div>
              </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold" style={{ color: selectedDepartmentData?.color }}>
            ...
            {selectedDepartmentData?.description && (
              <div className="mt-4 p-4 bg-white rounded-lg border" style={{ borderColor: `${selectedDepartmentData?.color}40` }}>
                <p className="text-sm text-gray-600">{selectedDepartmentData.description}</p>
              </div>
            )}
          </div>
        </div>
