...
const LanguageToggle: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 flex flex-col transition-all duration-500 ${language === 'id' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
        <div className="w-full h-1/2 bg-red-500"></div>
        <div className="w-full h-1/2 bg-white"></div>
      </div>
      
      {/* Background - USA Flag */}
      <div className={`absolute inset-0 transition-all duration-500 ${language === 'en' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
      ...
      >
        <span className={`transition-all duration-300 font-medium ${isAnimating ? 'scale-0' : 'scale-100'}`}>
          {language === 'id' ? '🇮🇩' : '🇺🇸'}
        </span>
      </div>
