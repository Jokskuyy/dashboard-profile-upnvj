...
CREATE TABLE IF NOT EXISTS public.map_building_points (
    id BIGSERIAL PRIMARY KEY,
    map_id BIGINT NOT NULL REFERENCES public.campus_maps(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_map_nodes_map ON public.map_nodes(map_id);
CREATE INDEX IF NOT EXISTS idx_map_edges_map ON public.map_edges(map_id);
CREATE INDEX IF NOT EXISTS idx_map_edges_from ON public.map_edges(from_node_id);
CREATE INDEX IF NOT EXISTS idx_map_edges_to ON public.map_edges(to_node_id);
CREATE INDEX IF NOT EXISTS idx_map_buildings_map ON public.map_building_points(map_id);
CREATE INDEX IF NOT EXISTS idx_map_buildings_gedung ON public.map_building_points(gedung_id);
...
DROP POLICY IF EXISTS map_nodes_public_select ON public.map_nodes;
DROP POLICY IF EXISTS map_nodes_auth_all ON public.map_nodes;
CREATE POLICY map_nodes_public_select ON public.map_nodes
    FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY map_nodes_auth_all ON public.map_nodes
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS map_edges_public_select ON public.map_edges;
DROP POLICY IF EXISTS map_edges_auth_all ON public.map_edges;
CREATE POLICY map_edges_public_select ON public.map_edges
    FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY map_edges_auth_all ON public.map_edges
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS map_building_points_public_select ON public.map_building_points;
DROP POLICY IF EXISTS map_building_points_auth_all ON public.map_building_points;
